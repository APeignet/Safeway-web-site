    		...
    		<div id="menu_haut">
                ...
                <form>
                    <select onchange="location.href=this.value">
                        <option value="../controller/controller_gestion_gestionnaire_fr.php">Français</option>
                        <option value="../controller/controller_gestion_gestionnaire_en.php">English</option>
                    </select>
                </form>
            </div>


							<!-- nom AND prenom de la page -->
	            <h3>Gérer les conducteurs</h3>
				...
				<div class="conteneur_principal">
					...
					<!-- Rectangle blanc (nom AND prenom page) -->
					<div  class="pagetitle">
						<!-- Texte rectangle blanc -->
            <div class="pagetitletext">
              <h1>Bonjour, <span style="color: #5F5DFE"><?php echo $prenom.' '.$nom; ?></span> </h1>
              <h6>Retrouvez ici l'ensemble des résultats des conducteurs</h6>
           	</div>
                ...
                <g id="undraw_Hello_qnas" transform="translate(0 -3.773)">
                  ...
                  <path id="Path_1155" data-name="Path 1155" d="M366.087,292.714l44.492,42.274h49.113v-6.366L417.4,316.343l-36.177-44.675S364.8,281.818,366.087,292.714Z" transform="translate(-267.693 -182.64)" opacity="0.05"/>
                  <path id="Path_1156" data-name="Path 1156" d="M366.087,290.714l44.492,42.274h49.113v-6.366L417.4,314.343l-36.177-44.675S364.8,279.818,366.087,290.714Z" transform="translate(-267.693 -181.549)" fill="#6c63ff"/>
                ...
                <h2>Choisir un conducteur <a href="../controller/controller_inscription_fr.php" class="boutonajouter">Ajouter un conducteur</a> </h2>
              
                <div>
                  <div class="recherche_driver">		
				  	<?php
							
							$utilisateur = $bdd->query('SELECT * FROM utilisateur WHERE statut= "utilisateur"'); ?>
							<form method="GET">
							...
							<?php
							 if(isset($_GET['q']) AND !empty($_GET['q'])) {
								$q = ($_GET['q']);
								$utilisateur = $bdd->query('SELECT * FROM utilisateur WHERE statut= "utilisateur" AND nom LIKE "%'.$q.'%"');
								
								
							
							?>
							
							<?php if($utilisateur->rowCount() > 0) { ?>
								<ul>
								<?php while($a = $utilisateur->fetch()) { ?>
								<input type='button' class='boutonR' 
                      onclick="window.location.href='../controller/controller_selection_gestionnaire_fr.php?id=<?= $a['idUtilisateur'] ?>'" 
                      value='<?= $a['nom'] ?>'/>
								<?php } ?>
								</ul>
							<?php } else { ?>
              ...
              <tr>
                ...
                <div class="driver">
                  ...
                  <div class="driver-element">
					  ...
					  <img src='<?php echo '../view/public/images/'.$user['photo']."'"?>' >
                      </div>
                      
  
                      <h3> <?= $user['prenom'] ?> <br/> <?= $user['nom'] ?></h3>
                      <form action='../model/model_info_conducteur.php'  method='post'>
                      <input type='button' class='bouton' 
                      onclick="window.location.href='../controller/controller_selection_gestionnaire_fr.php?id=<?= $user['idUtilisateur'] ?>'" 
                      value='Afficher son profil'/> 
                      </form>
